...
; problem #4
(define (length-impl count xs)
    (if (null? xs)
        count
        (length-impl (+ count 1) (cdr xs))))
...
;                (cond
;                    ((null? ys) (reverse acc))
;                    ((pair? (car ys)) (impl acc (append (car ys) (cdr ys))))
;                    (else (impl (cons (car ys) acc) (cdr ys)))))))
;        (impl '() xs)))

(define (flatten-impl acc xs)
    (cond
        ((null? xs)
            (reverse acc))
        ((pair? (car xs))
            (flatten-impl acc (append (car xs) (cdr xs))))
        (else
            (flatten-impl (cons (car xs) acc) (cdr xs)))))
...
(define (flatten xs)
    ...
    ;                ((null? y) acc)
    ;                ((pair? y) 
    ;                    (impl (cdr y) (impl (car y) acc)))
    ;                (else
    ;                    (cons y acc))))))
    ;    (reverse (impl x '()))))


(writeln (flatten '(a (b (c d) e))))
;(writeln (flatten '(a (b (c '() d) e))))
...
(define (combination n xs)
    (cond
        ...
        (else
            (append
                (map
                    (lambda (y) (cons (car xs) y))
                    (combination (- n 1) (cdr xs)))
                (combination n (cdr xs))))))

(write (combination 3 '(a b c d e)))
(newline)
